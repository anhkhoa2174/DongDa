...
function renderSummary(values: TransactionFormValues) {
  const receivesUsd = values.transactionType === 'RECEIVE_USD';
    ...
    <div className="mt-2 mb-4 rounded-lg border border-teal-200 bg-teal-50 p-5">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="text-center">
          ...
          <div className="mt-1 text-xs text-slate-500">Applied {appliedRate.toLocaleString('vi-VN')}</div>
        </div>
        <div className="border-teal-200 text-center lg:border-x">
          ...
          <div className="mt-1 text-xs text-slate-500">MG implied {impliedRate.toLocaleString('vi-VN')}</div>
        </div>
        <div className="text-center">
...
const columns: ColumnsType<TransactionRecord> = [
  ...
  { title: 'MG Implied', dataIndex: 'mgRate', align: 'right', render: (value: number, record) => Number(value ?? (Number(record.paidVnd ?? record.receivedVnd ?? 0) / Number(record.paidUsd ?? record.receivedUsd ?? 1))).toLocaleString('vi-VN') },
  { title: 'Applied', dataIndex: 'appliedPaidRate', align: 'right', render: (value: number, record) => Number(value ?? (record.transactionType === 'RECEIVE_USD' ? activePaidRatesMock.paidSell : activePaidRatesMock.paidBuy)).toLocaleString('vi-VN') },
  {
    title: 'Profit',
    key: 'profit',
    align: 'right',
